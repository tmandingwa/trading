...
class PaperPosition:
    ...
    # new: trade management state
    r_dist: float                 # |entry - initial_sl|
    be_moved: bool = False        # moved SL to breakeven?
    mfv: float = 0.0              # max favorable for BUY (highest high)
    mfa: float = 0.0              # max favorable for SELL (lowest low)
...
class PaperEngine:
    ...
    Paper trading engine with:
      ...
      - cooldown after SL (blocks re-entry for N bars)
      - break-even at +1R
      - trailing after +1.5R (locks profit using R)
    ...
    def __init__(self, instrument: str, tf: str, db_path: str = "trades.db", max_trades: int = 200):
        ...
        # new: cooldown timestamp (when we can trade again)
    ...
    def _ensure_schema(self, conn: sqlite3.Connection) -> None:
        ...
        # lightweight migrations (add columns if DB exists older)
        ...
        # open_positions: r_dist, be_moved, mfv, mfa
    ...
    def load_from_db(self):
        ...
        with self._get_conn() as conn:
            ...
            cursor.execute("""
                SELECT side, entry_time, entry, sl, tp, units, reason, r_dist, be_moved, mfv, mfa
            ...
            if row:
                self.position = PaperPosition(
                    ...
                    units=float(row[5]),
                    reason=str(row[6] or ""),
                    r_dist=float(row[7] or abs(float(row[2]) - float(row[3]))),
                    be_moved=bool(int(row[8] or 0)),
                    mfv=float(row[9] or float(row[2])),
                    mfa=float(row[10] or float(row[2])),
                )
    ...
    def _tf_minutes(self) -> int:
        # Supports "M15", "M30", "H1" etc. (mainly used for cooldown)
    ...
    def build_strategy_cfg(self) -> Dict[str, Any]:
        """
        Call this when you call compute_state(...):
          cfg = {"TF": self.tf, **engine.build_strategy_cfg(), ... maybe HTF candles ...}
        """
        return {
    ...
    def _manage_open_position(self, candles: pd.DataFrame, plan: Optional[Dict[str, Any]] = None) -> None:
        """
        Called per new closed candle BEFORE checking TP/SL.
        Implements:
          - move SL to breakeven at +1R
          - trail after +1.5R (lock profit in R)
        plan may contain:
          - breakeven_at_r (default 1.0)
          - trail_after_r (default 1.5)
          - trail_lock_r (default 1.0)  # how much of move to lock in R
        """
        ...
        if pos.side == "BUY":
            ...
            if move_r >= trail_after:
                # lock in (move_r - trail_lock_r)R behind peak
                # Example: at 2R peak, lock 1R (SL = entry + 1R)
        ...
        # persist updated open position fields
        with self._get_conn() as conn:
            self._ensure_schema(conn)
            conn.execute("""
                INSERT OR REPLACE INTO open_positions
                  (instrument, tf, side, entry_time, entry, sl, tp, units, reason, r_dist, be_moved, mfv, mfa)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
    ...
    def update_on_new_closed_candle(self, candles: pd.DataFrame, plan: Optional[Dict[str, Any]] = None):
        """
        Called when your candle aggregator confirms a candle close.
        - manages open position (BE/Trail)
        - then checks TP/SL hits on the closed candle
        """
        ...
        # Manage open position first (BE/Trail) using the latest candle
        ...
        # Conservative ordering: if both hit, SL wins (worst-case)
        ...
        if outcome:
            ...
            # Cooldown after SL (default 2-4 bars depending on TF; prefer from plan/meta via maybe_enter)
            # Here we just keep any already-set cooldown; maybe_enter sets it on SL if you pass bars.
            # But if not set, we still set a safe default:
            if outcome == "SL":
                # default fallback
            ...
            with self._get_conn() as conn:
                ...
                conn.execute("""
                    INSERT INTO trade_history
                      (instrument, tf, side, entry_time, exit_time, entry, exit_px, sl, tp, units, pnl_usd, outcome, reason)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    self.instrument, self.tf,
    ...
    def maybe_enter(self, state: Dict[str, Any], candles: pd.DataFrame, balance: float, risk_pct: float):
        """
        Entry rules:
          - no entry if position open
          - no entry if cooldown active
          - require state ok and side BUY/SELL
          - uses state.plan entry/sl/tp
          - uses risk_pct for sizing
          - sets cooldown bars suggestion for next SL via state.meta if present
        """
        if candles is None or len(candles) < 2:
            return
        ...
        units = self.compute_units_quote_usd(balance=float(balance), risk_pct=float(risk_pct), entry=float(entry), sl=float(sl))
        if units <= 0:
            return
        ...
        # create position + initialize mfv/mfa
        pos = PaperPosition(
            side=str(side),
    ...
    # -----------------------------
    # Optional: call this after a trade closes if you want precise cooldown bars
    # (If you can update your caller to set it, do it. Otherwise defaults used.)
    # -----------------------------
    def apply_cooldown_from_state_meta(self, state: Dict[str, Any], last_exit_ts: pd.Timestamp):
        """
        If you want cooldown bars to match strategy preset exactly:
          call this after SL close with the state that generated the trade.
        """
        try:
